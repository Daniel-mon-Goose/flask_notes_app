...
Последовательность действий для создания и запуска контейнера, находясь в той же директории,
что и Dockerfile:

* создание образа контейнера командой `docker build -t notes_app:latest .`;
* создание самого контейнера командой `docker run --name notes_app -p 8000:5000 -d notes_app` (создаёт контейнер с именем `notes_app` и сопоставленным портом 8000);
